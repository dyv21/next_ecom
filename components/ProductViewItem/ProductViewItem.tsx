...
const ProductViewInfo = React.forwardRef<HTMLDivElement, ProductViewInfoProps>(
 ...
 description,
    ...
    const brandToUrl = (brand: string) => brand.toLowerCase().split(' ').join('_')

    return (
   <>
     <Breadcrumbs>
       <BreadcrumbItem><Link href={'/'}>Home</Link></BreadcrumbItem>
       <BreadcrumbItem><Link href={`/${brandToUrl(brand)}`}>{brand}</Link></BreadcrumbItem>
     ...
     >
       ...
       <div className="relative h-full w-full flex-none">
         ...
         {/* Main Image */}
         <Image alt={title} className="h-full w-full" radius="lg" src={selectedImage} />
         {/* Image Gallery */}
         <ScrollShadow
       ...
       <div className="flex flex-col">
         ...
         <div className="mt-4">
           ...
           <p className="line-clamp-3 text-medium text-default-500">{description}1</p>
         </div>
         {/*<RadioGroup*/}
         {/*  aria-label="Color"*/}
         {/*  classNames={{*/}
